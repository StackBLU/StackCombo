...
namespace StackCombo.Combos.PvP
{
	internal static class DNCPvP
	{
		...
		internal class DNCPvP_BurstMode : CustomComboClass
		{
			...
			protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
			{
				if (actionID is Cascade or Fountain or ReverseCascade or Fountainfall)
				{
					...
					byte acclaimStacks = GetBuffStacks(Buffs.Acclaim);
					...
					{
						return HasEffect(Buffs.Acclaim) && acclaimStacks < 4 ? WHM.Assize : HoningDance;
					}
