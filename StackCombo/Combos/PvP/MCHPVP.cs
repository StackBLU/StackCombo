...
namespace StackCombo.Combos.PvP
{
	internal static class MCHPvP
	{
		...
		internal class MCHPvP_BurstMode : CustomComboClass
		{
			...
			protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
			{
				if (actionID == BlastCharge)
				{
					...
					uint analysisStacks = GetRemainingCharges(Analysis);
					uint bigDamageStacks = GetRemainingCharges(OriginalHook(Drill));
