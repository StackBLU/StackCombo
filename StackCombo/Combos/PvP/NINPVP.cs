...
namespace StackCombo.Combos.PvP
{
	internal static class NINPvP
	{
		...
		internal class NINPvP_ST_BurstMode : CustomComboClass
		{
			...
			protected override uint Invoke(uint actionID, uint lastComboActionID, float comboTime, byte level)
			{
				...
				{
					Data.CooldownData threeMudrasCD = GetCooldown(ThreeMudra);
					Data.CooldownData fumaCD = GetCooldown(FumaShuriken);
					int bunshinStacks = HasEffect(Buffs.Bunshin) ? GetBuffStacks(Buffs.Bunshin) : 0;
		...
		internal class NINPvP_AoE_BurstMode : CustomComboClass
		{
			...
			protected override uint Invoke(uint actionID, uint lastComboActionID, float comboTime, byte level)
			{
				if (actionID == FumaShuriken)
				{
					...
					if (InMeleeRange())
					{
						return lastComboActionID == GustSlash
							? OriginalHook(AeolianEdge)
							: lastComboActionID == SpinningEdge ? OriginalHook(GustSlash) : OriginalHook(SpinningEdge);
					}
				}
