...
namespace StackCombo.Combos.JobHelpers
{
	...
	{
		private static bool HasEffect(ushort id)
		{
			return CustomComboFunctions.HasEffect(id);
		}

		private static float GetBuffRemainingTime(ushort effectid)
		{
			return CustomComboFunctions.GetBuffRemainingTime(effectid);
		}

		private static bool LevelChecked(uint id)
		{
			return CustomComboFunctions.LevelChecked(id);
		}

		private static float GetActionCastTime(uint actionID)
		{
			return CustomComboFunctions.GetActionCastTime(actionID);
		}

		private static uint GetRemainingCharges(uint actionID)
		{
			return CustomComboFunctions.GetRemainingCharges(actionID);
		}

		private static float GetCooldownRemainingTime(uint actionID)
		{
			return CustomComboFunctions.GetCooldownRemainingTime(actionID);
		}

		private static bool ActionReady(uint id)
		{
			return CustomComboFunctions.ActionReady(id);
		}

		private static bool CanSpellWeave(uint id)
		{
			return CustomComboFunctions.CanSpellWeave(id);
		}

		private static bool HasCharges(uint id)
		{
			return CustomComboFunctions.HasCharges(id);
		}

		private static bool TraitLevelChecked(uint id)
		{
			return CustomComboFunctions.TraitLevelChecked(id);
		}

		private static byte GetBuffStacks(ushort id)
		{
			return CustomComboFunctions.GetBuffStacks(id);
		}
		...
		internal class RDMMana : RDM
		{
			...
			internal static int ManaStacks
			{
				get
				{
					return Gauge.ManaStacks;
				}
			}

			internal static int Black
			{
				get
				{
					return AdjustMana(Gauge.BlackMana);
				}
			}

			internal static int White
			{
				get
				{
					return AdjustMana(Gauge.WhiteMana);
				}
			}
			...
			internal static int Min
			{
				get
				{
					return AdjustMana(Math.Min(Gauge.BlackMana, Gauge.WhiteMana));
				}
			}
			...
			internal static int Max
			{
				get
				{
					return AdjustMana(Math.Max(Gauge.BlackMana, Gauge.WhiteMana));
				}
			}
			...
			public static (bool useFire, bool useStone, bool useThunder, bool useAero, bool useThunder2, bool useAero2) CheckBalance()
			{
				...
				{
					if (blackmana <= whitemana || HasEffect(Buffs.VerstoneReady))
					{
						useThunder = true;
					}

					if (whitemana <= blackmana || HasEffect(Buffs.VerfireReady))
					{
						useAero = true;
					}
					...
					&& !HasEffect(Buffs.Acceleration))
				{
					...
					if (blackmana <= whitemana && VerFireReady)
					{
						useFire = true;
					}

					if (whitemana <= blackmana && VerStoneReady)
					{
						useStone = true;
					}
				...
				if (LevelChecked(Verthunder2)
					...
					&& !HasEffect(Buffs.Acceleration))
				{
					if (blackmana <= whitemana || !LevelChecked(Veraero2))
					{
						useThunder2 = true;
					}
					else
